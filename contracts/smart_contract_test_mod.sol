...
contract Four_In_A_Chain {
    ...
    //function getBoard(uint ID) public view returns (uint8[7][6] memory board) {
    //    return games[ID].board;
   // }

    function getBoard(uint ID) public view returns (uint[6] memory firstcolumn, uint[6] memory secondcolumn, uint[6] memory thirdcolumn, uint[6] memory fourthcolumn, uint[6] memory fifthcolumn, uint[6] memory sixthcolumn, uint[6] memory seventhcolumn ){
        Game storage g = games[ID];
        for(uint8 i=0; i<=5; i++){
            firstcolumn[i] = g.board[i][0];
            secondcolumn[i] = g.board[i][1];
            thirdcolumn[i] = g.board[i][2];
            fourthcolumn[i] = g.board[i][3];
            fifthcolumn[i] = g.board[i][4];
            sixthcolumn[i]= g.board[i][5];
            seventhcolumn[i] = g.board[i][6];
        }
    }
    
    //Tried to change it to see if that would change some things. I don't think the changes are actually relevant
    //and we can probably keep the original one. (i just wanna go to bed and not risk some things not working now)
